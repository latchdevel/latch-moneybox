...
class Latch(object):
    ...
    API_PROXY = None;
    API_PROXY_PORT = None;
    API_CHECK_STATUS_URL = "/api/1.0/status";
    API_PAIR_URL = "/api/1.0/pair";
    API_PAIR_WITH_ID_URL = "/api/1.0/pairWithId";
    API_UNPAIR_URL = "/api/1.0/unpair";
    ...
    def _http_get(self, url, xHeaders=None):
        ...
        if Latch.API_PROXY != None:
            if Latch.API_HTTPS:
                conn = httplib.HTTPSConnection(Latch.API_PROXY, Latch.API_PROXY_PORT)
                conn.set_tunnel(Latch.API_HOST, Latch.API_PORT) 
            else: 
                conn = httplib.HTTPConnection(Latch.API_PROXY, Latch.API_PROXY_PORT)
                url = "http://" + Latch.API_HOST + url                  
        else:
            if Latch.API_HTTPS:
                conn = httplib.HTTPSConnection(Latch.API_HOST, Latch.API_PORT)
            else: 
                conn = httplib.HTTPConnection(Latch.API_HOST, Latch.API_PORT)
